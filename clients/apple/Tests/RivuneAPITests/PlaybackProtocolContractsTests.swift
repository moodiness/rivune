...
final class PlaybackProtocolContractsTests: XCTestCase { private let titleId = UUID(uuidString: "11111111-1111-4111-8111-111111111111")!
...
func testDiscoveryMetadataAndRequiredContentModelsDecode() throws {
    ...
    {"name":"Rivune","serverVersion":"22.0","protocolVersion":22,"apiBaseUrl":"/api/v1","setupRequired":false,"setupCompleted":true,"demoAvailable":false,"timezone":"UTC","interfaceLanguage":"en-US"}
    """.utf8))
...
func testMetadataMarkersAndSourcesRequestsUseCurrentQueryAndBody() async throws {
    let transport = ProtocolRecordingTransport()
...
func testCoordinationV22OperationResultAndOutgoingRoutes() async throws {
    let transport = ProtocolRecordingTransport()
    let client = try makeClient(transport: transport)
    let targetSession = UUID(uuidString: "77777777-7777-4777-8777-777777777777")!
    let operationId = UUID(uuidString: "99999999-9999-4999-8999-999999999999")!
    ...
    _ = try await client.sendPlaybackCommand(
        ...
        input: PlaybackCommandInput(
            ...
            positionMilliseconds: 1_000, mode: .handoff, targetRevision: 7))
    _ = try await client.playbackCommands(after: operationId)
    _ = try await client.reportPlaybackCommandResult(
        operationId: operationId,
        input: PlaybackCommandResultInput(status: .applied, code: .applied))
    _ = try await client.outgoingPlaybackCommand(operationId: operationId)
    ...
    let recommendations = try await client.localRecommendations(limit: 12, artworkShape: .landscape)
    ...
    XCTAssertEqual(requests.map { $0.url?.path }, [
        ...
        "/api/v1/playback/commands/incoming/\(operationId.uuidString.lowercased())/result",
        "/api/v1/playback/commands/outgoing/\(operationId.uuidString.lowercased())",
    ...
    XCTAssertEqual(query(requests[3])["after"], operationId.uuidString.lowercased())
    XCTAssertEqual(query(requests[11])["limit"], "12")
    XCTAssertEqual(query(requests[11])["artworkShape"], "landscape")
}

func testPlaybackRoomMemberUsesOpaqueMemberID() throws {
    let room = try JSONDecoder().decode(PlaybackRoom.self, from: Data(ProtocolRecordingTransport().roomJSON.utf8))
...
private func makeClient(transport: ProtocolRecordingTransport) throws -> RivuneAPIClient {
    try RivuneAPIClient(
        serverURL: URL(string: "https://example.test")!,
        transport: transport,
        credentialStore: TestCredentialStore(token: TokenPair(
            tokenType: "Bearer",
            accessToken: "access",
...
private struct TestCredentialStore: CredentialStore { let token: TokenPair
...
private final class ProtocolRecordingTransport: HTTPTransport, @unchecked Sendable { private let lock = NSLock()
...
func data(for request: URLRequest) async throws -> (Data, HTTPURLResponse) {
    ...
    if path.contains("/playback/commands/") {
        return response(request, body: Data(commandJSON.utf8))
    }
...
private var deviceJSON: String {
    "{\"sessionId\":\"55555555-5555-4555-8555-555555555555\",\"deviceId\":\"66666666-6666-4666-8666-666666666666\",\"name\":\"Device\",\"platform\":\"apple\",\"capabilities\":[\"remote-control\"],\"state\":{\"status\":\"idle\",\"positionMilliseconds\":0,\"durationMilliseconds\":0},\"revision\":7,\"current\":true,\"lastSeenAt\":\"2099-01-01T00:00:00Z\"}"
}

private var commandJSON: String {
    "{\"operationId\":\"99999999-9999-4999-8999-999999999999\",\"command\":\"load\",\"mode\":\"handoff\",\"targetRevision\":7,\"senderDeviceName\":\"Sender\",\"status\":\"applied\",\"resultCode\":\"applied\",\"createdAt\":\"2099-01-01T00:00:00Z\",\"expiresAt\":\"2099-01-01T00:02:00Z\"}"
}
