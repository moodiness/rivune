...
final class BrowseV20ContractsTests: XCTestCase {
    private let collectionId = UUID(uuidString: "11111111-1111-4111-8111-111111111111")!
    ...
    func testFullCollectionFolderAndDynamicResourceDecode() throws {
        let collection = try JSONDecoder().decode(CollectionList.self, from: Data(Self.collectionJSON.utf8)).collections[0]
        ...
        let folder = try JSONDecoder().decode(ResolvedCollectionFolder.self, from: Data(Self.folderJSON.utf8))
        XCTAssertEqual(folder.page, 2)
        XCTAssertTrue(folder.hasMore)
        XCTAssertEqual(folder.errors[0].code, .sourceTimeout)
        XCTAssertEqual(folder.items[0].raw, .object(["flag": .boolean(true), "signed": .signedInteger(-7), "unsigned": .signedInteger(9), "fraction": .floatingPoint(1.25), "nested": .array([.null, .string("ok")])]))

        let resource = try JSONDecoder().decode(AddonResourceResult.self, from: Data(Self.resourceJSON.utf8))
        ...
        XCTAssertEqual(try JSONDecoder().decode(JSONValue.self, from: Data("18446744073709551615".utf8)), .unsignedInteger(UInt64.max))
    }

    func testBrowseRequestsPreserveRoutesQueriesBodiesAndMutations() async throws {
        let transport = BrowseTransport()
        let client = try makeClient(transport)
        _ = try await client.resolveCollectionFolder(collectionId: collectionId, folderId: folderId, page: 2, limit: 50, language: "fr-FR", region: "CA")
        _ = try await client.searchAddonCatalogs(type: "tv anime/été", search: "café & tea", skip: 4, limit: 24, extras: [.init(name: "genre", value: "A/B"), .init(name: "genre", value: "日本")])
        _ = try await client.addonResource(addonId: addonId, resource: "meta/x", type: "tv anime", id: "id/日本?x=1", skip: 3, limit: 10, extras: [.init(name: "genre", value: "Drama"), .init(name: "genre", value: "Comedy")])
        _ = try await client.addonResources(resource: "catalog/x", type: "movie tv", id: "opaque/id", extras: [.init(name: "x", value: "1"), .init(name: "x", value: "2")])
        _ = try await client.resolveTitle(.init(mediaType: .tv, provider: "addon", resourceId: "channel/1", title: "News", sourceAddonId: addonId, sourceCatalogId: "live"))
        _ = try await client.library(mediaType: .tv, page: 3, pageSize: 100)
        _ = try await client.tvLibraryMembership([.init(sourceAddonId: addonId, resourceId: "channel/1")])
        ...
        XCTAssertEqual(requests.map(\.httpMethod), ["GET", "GET", "GET", "GET", "POST", "GET", "POST", "PUT", "DELETE", "GET", "DELETE"])
        XCTAssertEqual(requests[0].url?.path, "/api/v1/collections/\(collectionId.uuidString.lowercased())/folders/\(folderId.uuidString.lowercased())/items")
        XCTAssertEqual(URLComponents(url: requests[1].url!, resolvingAgainstBaseURL: false)?.percentEncodedPath, "/api/v1/addons/catalogs/search/tv%20anime%2F%C3%A9t%C3%A9")
        XCTAssertEqual(URLComponents(url: requests[2].url!, resolvingAgainstBaseURL: false)?.percentEncodedPath, "/api/v1/addons/\(addonId.uuidString.lowercased())/resource/meta%2Fx/tv%20anime/id%2F%E6%97%A5%E6%9C%AC%3Fx=1")
        XCTAssertEqual(URLComponents(url: requests[3].url!, resolvingAgainstBaseURL: false)?.percentEncodedPath, "/api/v1/addons/resources/catalog%2Fx/movie%20tv/opaque%2Fid")
        XCTAssertEqual(queryPairs(requests[0]), ["page=2", "limit=50", "language=fr-FR", "region=CA"])
        XCTAssertEqual(queryPairs(requests[1]), ["search=café & tea", "skip=4", "limit=24", "genre=A/B", "genre=日本"])
        ...
        let titleBody = try XCTUnwrap(JSONSerialization.jsonObject(with: try XCTUnwrap(requests[4].httpBody)) as? [String: Any])
        XCTAssertEqual(titleBody["mediaType"] as? String, "tv")
        XCTAssertEqual(titleBody["provider"] as? String, "addon")
        XCTAssertEqual((titleBody["sourceAddonId"] as? String)?.lowercased(), addonId.uuidString.lowercased())
    ...
    func testResourceURLAndProfileContextLifecycleIncludingRefresh() async throws {
        ...
        let externalURL = try await client.resolveResponseResourceURL("https://cdn.example.test/image.jpg")
        ...
        let notification = try XCTUnwrap(requests.first { $0.url?.path.hasSuffix("/auth/notifications") == true })
        XCTAssertEqual(notification.value(forHTTPHeaderField: "X-Rivune-Profile-Context"), "opaque-profile-context")
        let clear = try XCTUnwrap(requests.first { $0.url?.path.hasSuffix("/profiles/selection") == true })
        ...
        XCTAssertEqual(collectionRequests[0].value(forHTTPHeaderField: "X-Rivune-Profile-Context"), "opaque-profile-context")
        XCTAssertEqual(collectionRequests[1].value(forHTTPHeaderField: "X-Rivune-Profile-Context"), "opaque-profile-context")
        XCTAssertEqual(collectionRequests[2].value(forHTTPHeaderField: "X-Rivune-Profile-Context"), "opaque-profile-context")
    ...
    func testProfileContextIsRestoredByRecreatedClient() async throws {
        ...
        let selectingClient = try RivuneAPIClient(serverURL: server, transport: selectionTransport, credentialStore: store)
        ...
        let restoredRequest = try XCTUnwrap(restoredTransport.apiRequests().first { $0.url?.path.hasSuffix("/collections") == true })
        ...
        let refreshedClient = try RivuneAPIClient(serverURL: server, transport: refreshedTransport, credentialStore: store)
        ...
        let refreshedRequest = try XCTUnwrap(refreshedTransport.apiRequests().first { $0.url?.path.hasSuffix("/collections") == true })
        XCTAssertEqual(refreshedRequest.value(forHTTPHeaderField: "X-Rivune-Profile-Context"), "opaque-profile-context")

        try await refreshedClient.clearProfileSelection()
        let clearedTransport = BrowseTransport()
        let clearedClient = try RivuneAPIClient(serverURL: server, transport: clearedTransport, credentialStore: store)
        ...
        let clearedRequest = try XCTUnwrap(clearedTransport.apiRequests().first { $0.url?.path.hasSuffix("/collections") == true })
    ...
    func testAuthenticatedResponseCannotCrossProfileClear() async throws {
        ...
        XCTAssertFalse(transport.apiRequests().contains { $0.url?.path.hasSuffix("/auth/refresh") == true })
    }
    ...
    func testConcurrentProfileMutationsCannotReachTheServer() async throws {
        ...
        let selectRequests = transport.apiRequests().filter { $0.url?.path.hasSuffix("/select") == true }
    ...
    func testBrowseRequestDoesNotStartDuringProfileMutation() async throws {
        ...
        XCTAssertFalse(transport.apiRequests().contains { $0.url?.path.hasSuffix("/collections") == true })
        transport.releaseBlockedRequest()
        try await clear.value
    }

    private func makeClient(_ transport: BrowseTransport) throws -> RivuneAPIClient {
        try RivuneAPIClient(serverURL: URL(string: "https://example.test")!, transport: transport, credentialStore: BrowseCredentials(token: BrowseTransport.token(access: "access")))
    }
...
private actor BrowseCredentials: CredentialStore {
    ...
    func save(_ credentials: StoredCredentials, for issuer: URL) async throws { self.credentials = credentials }
...
private final class BrowseTransport: HTTPTransport, @unchecked Sendable {
    ...
    func failNextCollectionsWithUnauthorized() { lock.withLock { unauthorizedCollections = true; didRejectCollections = false } }
    ...
    func data(for request: URLRequest) async throws -> (Data, HTTPURLResponse) {
        ...
        if path == "/.well-known/rivune" { return response(request, body: Data("{\"name\":\"Rivune\",\"serverVersion\":\"20\",\"protocolVersion\":20,\"apiBaseUrl\":\"/api/v1\",\"setupRequired\":false,\"timezone\":\"UTC\",\"interfaceLanguage\":\"en-US\"}".utf8)) }
        ...
        if path.hasSuffix("/collections") {
            let reject = lock.withLock { () -> Bool in
                if unauthorizedCollections && !didRejectCollections { didRejectCollections = true; return true }
                return false
            }
            if reject { return response(request, status: 401, body: Data("{\"error\":{\"code\":\"expired\",\"message\":\"expired\"}}".utf8)) }
        ...
        if path.hasSuffix("/library") { return response(request, body: Data("{\"items\":[],\"page\":3,\"totalPages\":0,\"totalResults\":0}".utf8)) }
