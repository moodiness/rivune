...
final class DiscoveryCapabilitiesTests: XCTestCase {
  func testStableCapabilityIdentifiersAndOmittedCapabilities() throws {
    XCTAssertEqual(
      ...
      [
        "bounded-aggregate-resources", "profile-archives-v2", "request-correlation",
        "local-recommendations", "semantic-search", "playback-coordination",
        "playback-command-results", "addon-verifications",
      ]
    )
    ...
    let discovery = try JSONDecoder().decode(
      ...
      from: Data(
        """
        {"name":"Rivune","serverVersion":"22.0","protocolVersion":22,"apiBaseUrl":"/api/v1","setupRequired":false,"timezone":"UTC","interfaceLanguage":"en-US"}
        """.utf8))

    XCTAssertEqual(discovery.capabilities, [])
    XCTAssertFalse(discovery.supports(.profileArchivesV2))
  ...
  func testCapabilitiesRetainSafeUnknownsAndDiscardMalformedEntriesAndDuplicates() throws {
    ...
    let discovery = try JSONDecoder().decode(
      ...
      from: Data(
        """
        {"name":"Rivune","serverVersion":"22.0","protocolVersion":22,"apiBaseUrl":"/api/v1","setupRequired":false,"timezone":"UTC","interfaceLanguage":"en-US","capabilities":["profile-archives-v2","future-capability","profile-archives-v2","","Uppercase","-leading","trailing-","two--hyphens","contains/slash","\(tooLong)",17,null,{"identifier":"request-correlation"}]}
        """.utf8))

    XCTAssertEqual(discovery.capabilities, ["profile-archives-v2", "future-capability"])
    XCTAssertTrue(discovery.supports(.profileArchivesV2))
  ...
  func testCapabilitiesAreStableDeduplicatedAndCappedAt64() throws {
    let identifiers =
      (0..<70).map { "future-\($0)" } + [
        "future-0", DiscoveryCapability.profileArchivesV2.rawValue,
      ]
...
private struct DiscoveryTransport: HTTPTransport {
  func data(for request: URLRequest) async throws -> (Data, HTTPURLResponse) {
    let body = Data(
      """
      {"name":"Rivune","serverVersion":"22.0","protocolVersion":22,"apiBaseUrl":"/api/v1","setupRequired":false,"timezone":"UTC","interfaceLanguage":"en-US","capabilities":["profile-archives-v2","future-capability","profile-archives-v2","not_safe"]}
      """.utf8)
