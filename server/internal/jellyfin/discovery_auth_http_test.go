...
func TestAuthenticateByNameAcceptsEmptyPreAuthToken(t *testing.T) {
	fake, mux := newDiscoveryHTTPTestServer(t)
	request := httptest.NewRequest(http.MethodPost, "/Users/AuthenticateByName", strings.NewReader(`{"Username":"c3000000-0000-4000-8000-000000000003","Pw":"correct horse"}`))
	request.Header.Set("Content-Type", "application/json")
	request.Header.Set("Authorization", `MediaBrowser Client="Compatibility SDK", Device="Mobile%20Device", DeviceId="11111111-1111-4111-8111-111111111111", Version="1.0", Token=""`)
	response := httptest.NewRecorder()

	mux.ServeHTTP(response, request)

	if response.Code != http.StatusOK {
		t.Fatalf("login with empty pre-auth token status = %d, body = %s", response.Code, response.Body.String())
	}
	var result AuthenticationResult
	decodeCompatTestResponse(t, response, &result)
	if fake.loginCalls != 1 || fake.lastLogin.Username != "c3000000-0000-4000-8000-000000000003" ||
		fake.lastLogin.Password != "correct horse" || fake.lastLogin.Client.Client != "Compatibility SDK" ||
		fake.lastLogin.Client.Device != "Mobile%20Device" || result.AccessToken != fake.token || result.User.Id != discoveryProfileID {
		t.Fatalf("login request was not preserved: input=%+v result=%+v calls=%d", fake.lastLogin, result, fake.loginCalls)
	}
}
