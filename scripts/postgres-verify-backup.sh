...
BACKUP_FILE="$3"
case "${VERIFY_MODE}" in
  --expect-backup-id|--allow-legacy-manifest) [[ "${OPERATOR_EXPECTATION}" =~ ^[0-9a-f]{32}$ ]] || usage ;;
...
if [[ "${VERIFY_MODE}" == --allow-legacy ]]; then
  ...
  if [[ "${MANIFEST_FORMAT}" == v3-age ]]; then
    if [[ "${VERIFY_MODE}" == --allow-legacy-manifest ]]; then
  ...
  elif [[ "${VERIFY_MODE}" != --allow-legacy-manifest ]]; then
...
stream_authenticated_backup \
  | PGPASSWORD="${RESTORE_PASSWORD}" docker exec -i -e PGPASSWORD "${CONTAINER}" pg_restore \
