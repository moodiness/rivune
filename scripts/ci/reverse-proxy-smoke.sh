...
https_request --fail --silent --show-error \
  ...
  | docker run --rm -i "${PYTHON_IMAGE}" python -c 'import json,sys; value=json.load(sys.stdin); assert value["apiBaseUrl"] == "https://localhost/api/v1"; assert value["protocolVersion"] == 16'
