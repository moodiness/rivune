...
try {
    foreach ($invalidOrigin in @(
        ...
        Assert-Fails {
            & $script validate -Origin $invalidOrigin -Name 'Rivune' -Version '1.11.2'
        } "Unsafe discovery origin $invalidOrigin"
    }
    Assert-Fails {
        & $script validate -Origin 'https://media.example.com' -Name '' -Version '1.11.2'
    } 'Empty discovery name'
    Assert-Fails {
        & $script validate -Origin 'https://media.example.com' -Name 'Rivune' -Version 'latest'
    } 'Mutable discovery version'

    & $script validate -Origin 'https://media.example.com/' -Name 'Rivune Windows CI' -Version '1.11.2'
    & $script validate -Origin 'http://192.168.1.20:8080' -Name 'Rivune Windows CI' -Version '1.11.2'
    ...
    if ($statusOutput -notmatch "active as 'Rivune Windows CI' at https://media.example.com \(version 1.11.2, PID [0-9]+\)") {
    ...
    if ($config.origin -cne 'https://media.example.com' -or $config.name -cne 'Rivune Windows CI' -or
        $config.version -cne '1.11.2' -or [int] $config.port -ne 443) {
    ...
    if ($logOutput -notmatch 'active name=Rivune Windows CI origin=https://media.example.com version=1.11.2') {
