...
```sh
./rivune setup --public-url https://media.example.com --version 1.13.5
./rivune up
./rivune status
./rivune logs rivune
./rivune doctor
...
For public access, terminate TLS at Pangolin/Newt or another reverse proxy. Join the proxy to `rivune-edge` and target `http://rivune:8080`. Set `RIVUNE_PUBLIC_URL` to the exact externally visible HTTPS origin and set `RIVUNE_TRUSTED_PROXIES` to only the proxy's exact address or dedicated edge CIDR; never `0.0.0.0/0`, the LAN, or the database network. The proxy must replace, not append, client forwarding headers. `RIVUNE_PUBLIC_URL` is authoritative for the public browser origin. As a narrow local fallback, a direct request may instead use an allowed private IP literal only when its `Host` and `Origin` match exactly; DNS aliases and public IP alternatives remain rejected. Without `RIVUNE_PUBLIC_URL`, Rivune derives the origin from the direct request or from single `X-Forwarded-Proto` and `X-Forwarded-Host` values supplied by a trusted peer.

Browser login and device-code exchange use `/api/v1/auth/web/*`, not the native refresh route. JavaScript receives only a tab-local access token. The rotating refresh credential is a host-only `rivune_web_refresh` cookie with `HttpOnly`, `SameSite=Strict`, and `Path=/api/v1/auth/web/refresh`; it is `Secure` except on an accepted local HTTP origin and is never returned in JSON. Login, exchange, refresh, and logout require `X-Rivune-CSRF: 1` plus an exact `Origin`: either `RIVUNE_PUBLIC_URL`, or the same private IP literal and port as a direct request's `Host`. When the browser supplies `Sec-Fetch-Site`, it must be `same-origin`; browsers omit Fetch Metadata on non-trustworthy HTTP origins such as a private LAN address.

- `/live`: process liveness.
- `/ready`: application and PostgreSQL readiness.
- `/health`: protocol-compatible readiness response.

For trusted-LAN HTTP, set `RIVUNE_BIND_ADDRESS=0.0.0.0` and publish a host port. A public HTTPS `RIVUNE_PUBLIC_URL` can coexist with direct access at a literal private-IP origin such as `http://192.168.1.20:18080`; the direct request must use that exact IP and mapped port, not a LAN DNS alias. A LAN-only deployment may instead set `RIVUNE_PUBLIC_URL` to the private-IP origin. Browser cookies are host-scoped rather than port-scoped, so every HTTP service on that host IP joins the authentication trust boundary; if any service is untrusted, keep the port loopback-only and use the HTTPS domain. Restrict the host firewall and never forward the HTTP port from the router.
