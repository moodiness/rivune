...
git switch main
git pull --ff-only
git tag -a v1.10.0 -m "Rivune v1.10.0"
...
```sh
gh workflow run release-candidate.yml --ref main -f tag=v1.10.0
```
...
After all gates succeed, the workflow publishes one OCI manifest to `ghcr.io/moodiness/rivune` for exactly `linux/amd64` and `linux/arm64`, with provenance and an SBOM. A stable `v1.10.0` release receives:
...
```sh
image=ghcr.io/moodiness/rivune:1.10.0
