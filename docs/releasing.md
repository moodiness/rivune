...
The matching GitHub Release contains exactly `rivune-android-<version>.apk`, `rivune-android-<version>-corresponding-source.tar.gz`, canonical x64 `Rivune-x64.exe`, ARM64 `Rivune-arm64.exe`, the byte-identical legacy x64 bridge `Rivune.exe`, `rivune-update.json`, and the compatibility bridge `rivune-android-update.json`. GitHub publishes a SHA-256 digest for every release asset; no separate checksum-list asset is generated. `rivune-update.json` remains schema v2: common release metadata plus required `packages.android`, legacy-compatible x64 `packages.windows`, canonical x64 `packages.windowsX64`, and ARM64 `packages.windowsArm64` entries. The legacy and canonical x64 entries have different filenames but identical size and SHA-256. Older v1.8.0 clients continue using `packages.windows` and ignore the new package keys. The schema-v1 Android bridge is generated from, and validated byte-for-field against, the global Android entry so already-published v1.7.2 clients can reach the global feed. The universal APK uses application ID `io.rivune.app`, requires Android 8.0 or newer, and records its signing fingerprint. Both Windows architectures target Windows 10 build 19041 or newer. The executables are intentionally unsigned, so public users may see a Microsoft Defender SmartScreen unknown-publisher warning and must compare the selected asset with GitHub's published SHA-256 before running it. The corresponding-source archive contains the exact Rivune revision and complete authenticated native and JVM source graph needed to rebuild the GPL-enabled Android work.
...
The first policy assertion requires exactly the `linux/amd64` and `linux/arm64` runnable manifests and no others. The second requires one OCI attestation manifest for each runnable platform, as emitted by the workflow's `provenance: mode=max` and `sbom: true` settings. Record the immutable index digest and assertion outputs as the post-release attestation. Confirm in the GitHub UI that the Release points to the same annotated tag, includes the curated notes, and contains exactly the seven assets above. Inspect GitHub's asset digests with `gh release view <tag> --json assets --jq '.assets[] | [.name,.digest] | @tsv'`, inspect the APK certificate with Android SDK `apksigner verify --verbose --print-certs`, and confirm that `Rivune.exe` and `Rivune-x64.exe` are byte-identical x64 PE files with machine `0x8664`, `Rivune-arm64.exe` has ARM64 PE machine `0xAA64`, all three report tag-derived file/product versions, and all return `NotSigned` from `Get-AuthenticodeSignature`. Smoke-launch each canonical downloaded executable on its native architecture before announcing the release.
