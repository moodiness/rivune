...
Windows release executables are deliberately unsigned and require no signing secret. The Windows build job emits self-contained x64 and ARM64 executables, verifies their PE architecture, exact tag-derived `ProductVersion`, size, filenames, and `NotSigned` Authenticode status, then passes those immutable bytes directly to manifest generation.

At runtime, the Windows updater accepts only the architecture-specific `moodiness/rivune` GitHub asset URL recorded by the validated release manifest. It verifies the exact size, SHA-256, and `ProductVersion` after download and again after staging before replacing the portable executable. These checks detect corruption and mismatched release bytes, but an unsigned executable has no publisher identity independent of GitHub; the UI and release notes disclose that limitation, and SmartScreen may warn.
...
git switch main
git pull --ff-only
git tag -a v1.11.5 -m "Rivune v1.11.5"
git push origin v1.11.5
```
...
```sh
gh workflow run release-candidate.yml --ref main -f tag=v1.11.5
```
...
After all gates succeed, the workflow publishes one OCI manifest to `ghcr.io/moodiness/rivune` for exactly `linux/amd64` and `linux/arm64`, with provenance and an SBOM. A stable `v1.11.5` release receives:
