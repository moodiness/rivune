...
cd rivune
./rivune setup --public-url https://media.example.com --version 1.11.4
./rivune up
./rivune doctor
```

Omit `--public-url` for a loopback-only installation. `--version` is required and accepts only an exact stable numeric release such as `1.11.4`; mutable image tags such as `latest` are rejected so a fresh install is reproducible. `./rivune help` lists explicit wrappers for lifecycle, logs, diagnostics, authenticated backup verification and restore, plus a host-supervised backup scheduler that verifies every archive through a disposable restore before retention pruning. The command always resolves the repository root before loading Compose or environment files.
