...
- episode entries returned by `/continue-watching` may include the optional `mappingProvider`, `episodeOrderId`, and `metadataSeasonId` continuation context as an all-or-none set. The set identifies a TVDB episode-order variant: `mappingProvider` is `tvdb`, `episodeOrderId` is its positive decimal order ID, and `metadataSeasonId` is the opaque mapped season ID. Canonical-hierarchy entries omit all three fields. This additive response context remains protocol 22 because clients ignore unknown optional properties;
